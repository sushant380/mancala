...
public class GameControllerTest {
    ...
    this.mockMvc
        ...
        .andExpect(
            mvcResult -> {
              String json = mvcResult.getResponse().getContentAsString();
              JsonNode actualObject = mapper.readTree(json);
              assertThat(actualObject.findValue("message").asText())
                  .isEqualTo("Game not found. May be its finished or deleted");
            });
  }
    ...
    this.mockMvc
        ...
        .andExpect(
            mvcResult -> {
              ...
              JsonNode actualObject = mapper.readTree(json);
              assertThat(actualObject.findValue("message").asText())
                  .isEqualTo("Game is running or finished. You can not join the game");
            });
  }
  ...
  void joinGame() throws Exception {
    ...
    when(gameService.joinGame(gameDto.get_id(), PLAYER_2)).thenReturn(gameDto);
    this.mockMvc
        .perform(
            put("/games/" + gameDto.get_id() + "/join")
                .header(
        ...
        .andExpect(
            mvcResult -> {
              String json = mvcResult.getResponse().getContentAsString();
              JsonNode actualObject = mapper.readTree(json);
              assertThat(actualObject.findPath("players").get(1).findValue("_id").asText())
                  .isEqualTo(PLAYER_2);
            });
  }
  ...
  void makeMove() throws Exception {
    ...
    when(gameService.makeMove(gameDto.get_id(), PLAYER_1, gameDto.getPits().get(0).getPitId()))
        .thenReturn(gameDto);
    this.mockMvc
        .perform(
        ...
        .andExpect(
            mvcResult -> {
              String json = mvcResult.getResponse().getContentAsString();
              JsonNode actualObject = mapper.readTree(json);
              assertThat(actualObject.findPath("pits").get(0).findValue("pables").asInt())
                  .isEqualTo(0);
            });
  }
  ...
  void moveException() throws Exception {
    when(gameService.makeMove(gameDto.get_id(), PLAYER_1, 1))
        .thenThrow(new InvalidPlayerMoveException());
    this.mockMvc
        .perform(
            put("/games/" + gameDto.get_id() + "/pits/" + gameDto.getPits().get(0).getPitId())
                .header(
